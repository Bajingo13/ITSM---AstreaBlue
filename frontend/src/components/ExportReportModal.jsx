  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className="w-full max-w-xl overflow-hidden rounded-[28px] border border-blue-100 bg-white shadow-2xl">
        ...
        <footer className="flex justify-end gap-3 border-t border-blue-100 bg-slate-50 px-6 py-4"><button type="button" onClick={onClose} className="rounded-xl border border-slate-200 bg-white px-5 py-3 text-sm font-black text-slate-700 hover:bg-slate-100">Cancel</button><button type="button" disabled={busy} onClick={onExport} className="astrea-export-primary inline-flex items-center gap-2 rounded-xl border border-blue-700 bg-blue-600 px-5 py-3 text-sm font-black text-white shadow-sm transition hover:bg-blue-700 disabled:opacity-60"><Download size={17}/>{busy ? "Preparing..." : "Continue"}</button></footer>
      </section>
    </div>
  );
}
