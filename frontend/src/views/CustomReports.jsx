...
const initial = { date_from: "", date_to: "", branch_id: "", department: "", priority: "", category_id: "", status: "", technician_id: "" };
const emptyOptions = { branches: [], departments: [], priorities: [], categories: [], statuses: [], technicians: [] };

function SelectFilter({ label, value, onChange, options, valueKey = "value", labelKey = "label" }) {
  return <label className="text-xs font-bold text-slate-600">{label}<select value={value} onChange={onChange} className="mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-blue-400"><option value="">All</option>{options.map((option) => { const primitive = typeof option === "string"; const optionValue = primitive ? option : option[valueKey]; return <option key={optionValue} value={optionValue}>{primitive ? option : option[labelKey]}</option>; })}</select></label>;
}
...
export default function CustomReports() {
  ...
  const query = useCallback((format) => { const params = new URLSearchParams(Object.entries(filters).filter(([, value]) => value)); if (format) params.set("format", format); return params.toString(); }, [filters]);
  const update = (key) => (event) => setFilters((current) => ({ ...current, [key]: event.target.value }));

  useEffect(() => {
    let active = true;
    fetch(`${API_URL}/api/v1/analytics/report-options`, { headers: authHeaders() }).then(async (response) => { const body = await response.json(); if (!response.ok) throw new Error(body.message); if (active) setOptions(body.data); }).catch((requestError) => { if (active) setError(requestError.message); });
    return () => { active = false; };
  }, []);
