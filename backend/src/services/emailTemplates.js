/**
 * emailTemplates.js
 * Shared deployment-aware email template blueprint.
 */
...
function getEmailBrandName() {
  const configured = String(process.env.EMAIL_BRAND_NAME || process.env.COMPANY_NAME || "AstreaBlue").trim() || "AstreaBlue";
  return /\bitsm\b/i.test(configured) ? configured : `${configured} ITSM`;
}

function generateEmailHtml(title, bodyContent) {
  const brandName = escapeTemplateText(getEmailBrandName());
