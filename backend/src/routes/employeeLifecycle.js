...
router.post("/cases", async (req, res) => {
  ...
  try {
    ...
    if (!relatedTicketId) {
      ...
      let ticketDescription = `${type} lifecycle request for ${subjectName}. Complete and verify the operational checklist in lifecycle case ${caseNumber}.`;
      if (type === "Onboarding") {
        await client.query(
          `INSERT INTO ticket_categories (category_name,description)
           VALUES (
             'Employee Lifecycle - Onboarding',
             'Internal coordination tickets generated by the Employee Lifecycle onboarding workflow.'
           )
           ON CONFLICT (category_name)
           DO UPDATE SET description=EXCLUDED.description`
        );
        categoryName = "Employee Lifecycle - Onboarding";
        const branchResult = await client.query(
          `SELECT branch_name FROM branches WHERE branch_id=$1`,
          [branchId]
        );
        ticketDescription = buildOnboardingTicketDescription({
          caseNumber,
          employeeName: subjectName,
          branchName: branchResult.rows[0]?.branch_name,
          department: employee?.department || subjectDepartment,
          jobTitle: subjectJobTitle,
          contactEmail: employee?.personal_email || employee?.email || subjectContactEmail,
          startDate: req.body.subject_start_date,
          notes: normalizeOptionalText(req.body.notes, 5000),
        });
      }
