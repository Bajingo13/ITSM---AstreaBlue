...
async function sendInviteResponse({
  ...
  try {
    ...
    return res.status(201).json({
      success: true,
      email_sent: false,
      warning: `Invite Created. Email Failed: ${err.message}`,
      invitation: {
        ...invitation,
        role: inviteRole.role_name,
      },
...
router.post("/:id/resend", async (req, res) => {
  try {
    ...
    if (user.invite_status === INVITE_STATUSES.REVOKED) {
      return res.status(400).json({ success: false, error: "Invitation revoked. Reactivate before resending." });
    }
    if (![INVITE_STATUSES.PENDING, INVITE_STATUSES.EXPIRED].includes(user.invite_status)) {
      return res.status(400).json({ success: false, error: "Only pending or expired invitations can be resent." });
    }
